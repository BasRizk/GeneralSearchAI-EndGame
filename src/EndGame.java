...
public class EndGame extends GeneralSearchProblem {
	...
	Node newNode;
	Integer bestPastSimilarNodeCost;
	 ...
	 * 
	 * this class for example relays on the Node class, which is basically a generic
	 * search tree node, however, as the state of the node is simply a generic
	 * HashMap <String, ?>, hence, we can have our own format to this state second
	 * term of the pair to afford our problem state;
	 * 
	 * therefore, our EndGame state contains:
	 * 
	 * iX, iY (Location of Iron Man) (s1C)ollected, s2C., s3C., s4C., s5C., s6C.,
	 * (w1K)illed, w2K, w3k, w4k, w5k
	 * 
	 * The damage taken by Iron man at a certain node will be represented by the
	 * path cost to that node.
	 * 
	 * @param initialState
	 */
	public EndGame(int[] ironmanPos, int gridWidth, int gridHeight, int[] worriorsPos, int[] stonesPos,
			int[] thanosPos) {
